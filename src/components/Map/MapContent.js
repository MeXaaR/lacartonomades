...
const MapContent = props => {
  ...
  const [{ selected, viewport = DEFAULT_VIEWPORT, refresh }, dispatch] =
    useMapContext();
  ...
  const handleCreatePlace = () => {
    ...
    history.push(
      `/newplace?lat=${newPlaceMarker.lat}&lng=${newPlaceMarker.lng}`
    );
    setNewPlaceMarker();
  };
  ...
  const iconFinder = place => {
    const currentCateg =
      place.category.find(c => selected.indexOf(c) > -1) || place.category[0];
      ...
      places.map(place => (
        ...
        >
          <Tooltip
            direction="top"
            className={
              place.category.find(c => selected.indexOf(c) > -1) ||
              place.category[0]
            }
  ...
  const newPlaceMarkerComp = useMemo(
    () =>
      newPlaceMarker ? (
        <Marker
